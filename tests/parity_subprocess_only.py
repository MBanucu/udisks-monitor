...
# ── startup tests (subprocess only) ───────────────────────────────

@unittest.skipUnless(udisksctl_available(), 'udisksctl not available')
class TestStartupSubprocess(unittest.TestCase):
...
# ── subscribers tests (subprocess only) ───────────────────────────

@unittest.skipUnless(udisksctl_available(), 'udisksctl not available')
class TestSubscribersSubprocess(unittest.TestCase):
...
@unittest.skipUnless(udisksctl_available(), 'udisksctl not available')
class TestEventsSubprocess(unittest.TestCase):
...
# ── parity tests (subprocess only, two runs compared) ─────────────
...
@unittest.skipUnless(udisksctl_available(), 'udisksctl not available')
class TestParitySubprocessOnly(unittest.TestCase):

    BACKEND = 'subprocess'

    def test_emits_all_event_types(self):
        events = _collect_events(self.BACKEND)
        self.assertIsNotNone(events, 'subprocess backend failed to connect')
        seen = {type(e) for e in events}
        for et in ALL_PARITY_TYPES:
            self.assertIn(et, seen,
                          f'{et.__name__} missing from subprocess backend '
