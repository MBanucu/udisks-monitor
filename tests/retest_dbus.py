...
def make_image():
    ...
    try:
        ...
        os.unlink(path)
        raise RuntimeError(f"loop-setup FAILED rc={r.returncode}: {r.stderr.strip()[:300]}")
...
def one_cycle():
    ...
    try:
        ...
        if not got:
            return None, f"no InterfaceAdded (waited {ia_elapsed:.0f}ms, ready={t_ready:.0f}ms)"
        ...
        mon.stop()
        mon.join(timeout=5)
    return events, f"ready={t_ready:.0f}ms ia={ia_elapsed:.0f}ms signals={len(events)}"

class TestRetestDbus10x(unittest.TestCase):
